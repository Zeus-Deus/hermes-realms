...
def test_failed_stop_requires_fresh_terminal_observation(
    ...
    try:
        ...
        retained = manager.registry.get(record["id"])
        assert retained["status"] == "stopped"
        assert retained["session_id"] == record["session_id"]
        assert not Path(record["runtime_dir"]).exists()
        # ExecStopPost may still be finishing after the app scope is gone.
        deadline = time.monotonic() + 10
        while True:
            states = {key: lifecycle.scope_info(record[key])["ActiveState"]
                      for key in ("scope", "guardian_unit")}
            if all(state in ("inactive", "failed") for state in states.values()):
                break
            assert time.monotonic() < deadline, states
            time.sleep(0.05)
        manager.delete(record["id"], session_id=record["session_id"])
